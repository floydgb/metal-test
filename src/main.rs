...
pub fn gpu_dot(a: &[f32], b: &[f32], gpu: &GPU) -> f32 {
    ...
    let buf_result = gpu_set_bufs(Arc::clone(&gpu.dev), encoder, a, b);
...
pub fn gpu_run(command_buffer: &CommandBufferRef, encoder: &ComputeCommandEncoderRef, len: usize) {
    ...
    encoder.dispatch_thread_groups(gpu_1d_size(threadgroups), gpu_1d_size(threads_per_threadgroup));
    encoder.end_encoding();
    command_buffer.commit();
    command_buffer.wait_until_completed();
}

pub fn gpu_set_bufs(dev: Arc<Device>, enc: &ComputeCommandEncoderRef, a: &[f32], b: &[f32]) -> Buffer {
...
pub fn gpu_1d_size(width: u64) -> MTLSize {
